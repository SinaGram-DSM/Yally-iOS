...
class TimeLineAPI {
    ...
    func createPost(_ sound: URL, _ content: String, _ img: Data?, _ hashtag: [String]) {
        print(content)
        print(hashtag)
        print(sound)
        print(img)
//        httpClient.post(.createPost, params: ["sound":sound, "content":content, "img":img ?? "허허", "hashtag":hashtag]).map { response, _  -> StatusCode in
//            switch response.statusCode {
//            case 201:
//                return .ok1
//            default:
